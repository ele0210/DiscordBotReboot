...
async def on_voice_state_update(member, before, after):
  if before.channel != after.channel:
    if after.channel and len(
        after.channel.members) == 1 and channel_current is not None:
...
async def task_onehour():
  ...
  now_hour = datetime.datetime.now(
    datetime.timezone(datetime.timedelta(hours=9))).strftime('%H')
  now_weekday = datetime.datetime.now(
    datetime.timezone(datetime.timedelta(hours=9))).strftime('%A')
...
async def play_gacha(author):
  ...
  if gacha <= 0:
    ...
    # 船降りろ
    await channel_current.send(member_list[random.randint(0, limit)].nick +
                               '、お前もう船降りろ')
  elif gacha <= 5:
    # ホモ
    await channel_current.send(member_list[random.randint(0, limit)].nick +
    ...
    # 給食
    await channel_current.send(member_list[random.randint(0, limit)].nick +
                               'は給食した。' + str(random.randint(1, 36)) + 'ヶ月休み')
  elif gacha <= 11:
    # 給料
    await channel_current.send(member_list[random.randint(0, limit)].nick +
                               'の来月の給料が' + str(random.randint(1, 300000)) +
                               '円になった')
  elif gacha <= 15:
    # 仕事しろ
    ...
    await channel_current.send(
      'https://www.amazon.co.jp/s?k=2021%E6%9C%80%E6%96%B0&rh=n%3A3210981&dc&__mk_ja_JP=%E3%82%AB%E3%82%BF%E3%82%AB%E3%83%8A&qid=1622905453&rnid=2321267051&ref=sr_nr_n_1'
    )
  elif gacha <= 76:
    # Steam
    await channel_current.send(
    ...
    # もこう
    await channel_current.send(
      'https://www.youtube.com/user/mokoustream/featured')
  elif gacha <= 88:
    # ヨシダヨシオ
    await channel_current.send(
    ...
    # 説
    await channel_current.send('野獣先輩' +
                               member_list[random.randint(0, limit)].nick +
                               '説')
  elif gacha == 99:
    # あたり
